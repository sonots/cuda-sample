...
public:
  CArray(T* data, ptrdiff_t size, ptrdiff_t* shape, ptrdiff_t* strides) : data_(data), size_(size) {
...
public:
  ...
  CArray(T* data, ptrdiff_t size) : data_(data), size_(size) {}
...
public:
  CIndexer(ptrdiff_t size, ptrdiff_t* shape) : size_(size) {
      for (int i = 0; i < ndim; ++i) {
          shape_[i] = shape[i];
      }
  }
...
public:
  ...
  CIndexer(ptrdiff_t size) : size_(size) {}
